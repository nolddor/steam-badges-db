...
describe('SteamCardExchange#getBadges()', () => {
  test('Throws "Malformed Response" error', async () => {
    ...
    await expect(SteamCardExchange.getBadges()).rejects.toThrow('Malformed response')
  })

  test('Get badges successfully', async () => {
    const badges = { ...badgeA, ...badgeB }
    getJSON.mockImplementationOnce(() => mockGetJSON(badges))
    await expect(SteamCardExchange.getBadges()).resolves.toStrictEqual(badges)
  })

  test('Skip 0-size badges', async () => {
    const badges = { ...badgeA, ...badgeC }
    getJSON.mockImplementationOnce(() => mockGetJSON(badges))
    await expect(SteamCardExchange.getBadges()).resolves.toStrictEqual(badgeA)
  })
})
